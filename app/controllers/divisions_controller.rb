class DivisionsController < ApplicationController
  ...
  def division
    ...
    divisions = Division.where.not(date:nil).includes(:division_info)
        ...
        case params[:sort]
          ...
          when "rebellions"
            ...
            if params[:last] == "3"
              ...
              min_date = Date.strptime((Division.where.not(date: nil).order(date: :asc).pluck(:date).last.strftime('%m.%Y')), '%m.%Y')
